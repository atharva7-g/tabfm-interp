...
def create_joint_dataset(num_samples: int = 4000, noise_std: float = 0.0, seed: int = 0) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    b = rng.uniform(0.5, 2.0, size=num_samples) * rng.choice([-1.0, 1.0], size=num_samples)
...
def build_paired_batch(a: np.ndarray, b: np.ndarray, c: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Given arrays of a,b,c of same length K, build paired inputs for mul and div.
    Returns: X_div[K,4], y_div[K], X_mul[K,4], y_mul[K]
    """
    x_div = np.stack([a, b, c, np.ones_like(a, dtype=np.float32)], axis=1).astype(np.float32)
    x_mul = np.stack([a, b, c, np.zeros_like(a, dtype=np.float32)], axis=1).astype(np.float32)
...
@torch.no_grad()
def run_and_cache_activations(regressor: TabPFNRegressor, model, X_np: np.ndarray, target_layers: List[str]) -> Dict[str, torch.Tensor]:
...
def train_linear_probe_for_feature(layer_acts: Dict[str, torch.Tensor], feature_values: np.ndarray, device: str = 'cuda', epochs: int = 200, lr: float = 1e-3) -> Dict[str, Dict[str, float]]:
    ...
    for layer_name, acts in sorted(layer_acts.items(), key=lambda kv: int(kv[0].split('_')[1])):
...
def run_experiment():
    ...
    X, y, y_mul_all, y_div_all = create_joint_dataset(num_samples=6000, noise_std=0.0, seed=0)
    ...
    probe_op_div = train_linear_probe_for_feature(acts_div, op_flag_div, device=device, epochs=150, lr=1e-3)
    probe_op_mul = train_linear_probe_for_feature(acts_mul, op_flag_mul, device=device, epochs=150, lr=1e-3)

    # a_over_b decodability (independent of op flag)
    a_over_b = (a / b).astype(np.float32)
    probe_div_feature = train_linear_probe_for_feature(acts_div, a_over_b, device=device, epochs=150, lr=1e-3)
    probe_mul_feature = train_linear_probe_for_feature(acts_mul, a_over_b, device=device, epochs=150, lr=1e-3)
    ...
    for name in layer_names:
        ...
        print(f"{name:>8}  {r_op_d:>12.4f}  {r_op_m:>12.4f}  {r_ab_d:>14.4f}  {r_ab_m:>14.4f}")
    ...
    for name in layer_names:
        ...
        y_mul_patched = apply_activation_patching(regressor, model, X_mul, {name: acts_div[name]})
        delta_to_div = np.mean(np.abs(y_mul_base - y_div) - np.abs(y_mul_patched - y_div))

        # Noising: patch mul activations into div run at this layer, measure movement away from y_div
        y_div_patched = apply_activation_patching(regressor, model, X_div, {name: acts_mul[name]})
        delta_from_div = np.mean(np.abs(y_div_base - y_div) - np.abs(y_div_patched - y_div))  # expect negative if harm
        ...
        print(f"{name}: DenoiseΔ→div={delta_to_div:+.5f}  NoiseΔ(from div)={delta_from_div:+.5f}  "
    ...
    worst_noise = min(results, key=lambda r: r['noise_delta_from_div'])  # most negative (largest harm)

    print("\n" + "-"*50)
    print(f"Best denoising (div→mul patched): {best_denoise['layer']}  Δ={best_denoise['denoise_delta_to_div']:+.5f}")
    print(f"Strongest noising harm (mul→div patched): {worst_noise['layer']}  Δ={worst_noise['noise_delta_from_div']:+.5f}")

    # 7) Final summary
    print("\nFINAL SUMMARY")
    print(f"Held-out MSE: {mse:.4f}, R²: {r2:.4f}")
    print("Interpretation guide:")
    print("- Denoising positive Δ indicates sufficiency at that layer to restore div behavior when patching into mul run.")
    print("- Noising negative Δ indicates necessity at that layer for correct div behavior.")
    print("- Prefer layers with high |Δ| where op-flag probe R² is low/moderate and a/b probe R² is high.")
