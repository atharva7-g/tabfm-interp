...
def train_linear_probe(
    ...
    epochs: int = 100,
    lr: float = 0.001,
) -> Tuple[LinearProbe, float, float, float]:
...
def main():
    ...
    print("Will sweep all layers and train probes to predict a/b from activations")

    # Create a single dataset
    X, y = create_ab_dataset(samples_per_dataset, noise_std=0.0)
    mean_a_over_b = np.mean(X[:, 0] * X[:, 1])
    print(
        f"Generated single dataset (samples={samples_per_dataset}), mean(a/b)={mean_a_over_b:.4f}"
    )
    ...
    for layer_name in layer_keys:
        ...
        probe_train_targets = torch.tensor(
            a_over_b_targets[idx_train], dtype=torch.float32
        )
        probe_eval_targets = torch.tensor(
            a_over_b_targets[idx_eval], dtype=torch.float32
        )
    ...
    for r in results:
        print(
            f"{r['layer']}: EvalR2={r['eval_r2']:.4f}, EvalLoss={r['eval_loss']:.4f}, TrueMean={r['true_mean']:.4f}, PredMean={r['pred_mean']:.4f}"
        )
    print("-" * 50)
    ...
    plt.tight_layout()
    plt.savefig(
        "r2_intermediate_value_probe_across_layers.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
