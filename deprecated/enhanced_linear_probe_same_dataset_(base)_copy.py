...
def train_linear_probe(
    ...
    # Initialize probe
    probe = LinearProbe(input_dim=activation_size, output_dim=1, hidden_dim=256).to(
        device
    )
...
def main():
    ...
    train_relationships = np.random.choice(all_relationship_ids, size=5, replace=False)
    test_relationships = [
        rid for rid in all_relationship_ids if rid not in train_relationships
    ]
    ...
    probe_train_mask = np.isin(
        test_relationship_ids, [random_key[i] for i in train_relationships]
    )
    ...
    # Test on remaining relationships (combined)
    print("\n" + "=" * 50)
    print("TESTING ON REMAINING RELATIONSHIPS (COMBINED)")
    print("=" * 50)

    # Collect all test activations and targets from all test relationships
    all_test_activations = []
    all_test_targets = []

    # Initialize test metrics variables
    test_mse = None
    test_loss = None
    test_r2 = None
    ...
    for test_relationship in test_relationships:
        ...
        if len(test_activations) == 0:
            print(
                f"  No samples found for relationship {test_relationship} (UID: {test_uid})"
            )
            continue
        ...
        all_test_activations.append(test_activations)
        all_test_targets.extend(test_targets)

    # Combine all test activations and targets
    if len(all_test_activations) > 0:
        combined_test_activations = torch.cat(all_test_activations, dim=0)
        combined_test_targets = np.array(all_test_targets)

        print(f"Combined test activations shape: {combined_test_activations.shape}")
        print(f"Combined test targets shape: {combined_test_targets.shape}")
        print(f"Test relationships included: {test_relationships}")

        # Flatten activations
        activation_flat = (
            combined_test_activations.view(-1, activation_size).to(device).float()
        )
        ...
        test_targets_tensor = torch.tensor(
            combined_test_targets, dtype=torch.float32
        ).to(device)
        test_loss = nn.MSELoss()(predictions, test_targets_tensor)
        test_r2 = r2_score(combined_test_targets, predictions.cpu().numpy())
        test_mse = mean_squared_error(combined_test_targets, predictions.cpu().numpy())

        print("\nOverall Test Metrics:")
        print(
            f"  {TARGET_LAYER}: MSE={test_mse:.4f}, Loss={test_loss.item():.4f}, R2={test_r2:.4f}"
        )
    else:
        print("  No test samples found for any test relationship")
