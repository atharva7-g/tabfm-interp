...
def main():
    ...
    if len(all_test_activations) > 0:
        ...
        # Flatten activations
        activation_flat = combined_test_activations.view(-1, activation_size).to(device).float()
        ...
        test_targets_tensor = torch.tensor(combined_test_targets, dtype=torch.float32).to(device)
        ...
        print(f"  {TARGET_LAYER}: MSE={test_mse:.4f}, Loss={test_loss.item():.4f}, R2={test_r2:.4f}")
    else:
