...
def create_mul_plus_c_dataset(
    num_samples: int = 1000, noise_std: float = 0.0
...
def create_div_plus_c_dataset(
    num_samples: int = 1000, noise_std: float = 0.0
) -> Tuple[np.ndarray, np.ndarray]:
    """Dataset with columns [a, b, c] and target y = a / b + c. b is kept away from zero."""
    a = np.random.randn(num_samples)
    b = np.random.uniform(0.5, 2.0, size=num_samples) * np.random.choice(
...
def create_mul_plus_cd_dataset(
    num_samples: int = 1000, noise_std: float = 0.0
) -> Tuple[np.ndarray, np.ndarray]:
...
def train_linear_probe(
    ...
    activation_eval: torch.Tensor,
    target_eval: torch.Tensor,
    device: str = "cuda",
    epochs: int = 100,
    lr: float = 0.001,
    hidden_layers: Optional[List[int]] = None,
) -> Tuple[ProbeMLP, float, float, float]:
    ...
    # Initialize probe
    probe = ProbeMLP(
        input_dim=activation_size, output_dim=1, hidden_layers=hidden_layers
    ).to(device)
...
def main():
    ...
    for config in experiment_configs:
        ...
        print(
            f"Generated dataset (samples={samples_per_dataset}), hidden term mean={hidden_stat:.4f}"
        )
        ...
        activations_test = extract_activations(
            regressor, regressor.model_, X_test, device
        )
        ...
        for layer_name in layer_keys:
            ...
            probe_train_targets = torch.tensor(
                target_values[idx_train], dtype=torch.float32
            )
            probe_eval_targets = torch.tensor(
                target_values[idx_eval], dtype=torch.float32
            )
            ...
            print(
                f"{layer_name}: TrainLoss={train_loss:.4f}, EvalLoss={eval_loss:.4f}, EvalR2={eval_r2:.4f}, TrueMean={true_mean:.4f}, PredMean={pred_mean:.4f}"
            )
        ...
        print("-" * 60)
        print(
            f"Best layer by Eval R2 for {config['name']}: {best['layer']} (EvalR2={best['eval_r2']:.4f})"
        )
    ...
    for summary in experiment_summaries:
        print(
            f"{summary['name']}: TabPFN MSE={summary['mse']:.4f}, R²={summary['r2']:.4f}, "
...
def run_probe_complexity_experiment(
    device: str, samples_per_dataset: int, layer_name: str = "layer_8"
):
    ...
    for relation in relation_configs:
        ...
        if layer_name not in activations_test:
            print(
                f"Layer {layer_name} not found. Available layers: {list(activations_test.keys())}"
            )
            continue
        ...
        probe_eval_acts = layer_acts[idx_eval]
        probe_train_targets = torch.tensor(
            hidden_targets[idx_train], dtype=torch.float32
        )
        ...
        for config in hidden_layer_configs:
            ...
            config_label = (
                f"{len(config)} hidden layers" if config else "0 hidden layers"
            )
            print(f"{config_label}: EvalR2={eval_r2:.4f}")

        plot_data.append({"label": relation["plot_label"], "r2": complexity_results})
    ...
    for data in plot_data:
        plt.plot(x_values, data["r2"], marker="o", linewidth=2, label=data["label"])
