...
def train_variable_complexity_probe(
    activation_train: torch.Tensor,
    activation_test: torch.Tensor,
...
def main():
    ...
    for layer_name in layer_names:
        ...
        all_activations_at_layer = torch.cat(
            (all_activations_at_layer[:, :3, :], all_activations_at_layer[:, 4:, :]),
            dim=1,
        ...
        activation_train, activation_test, probe_target_train, probe_target_test = (
            train_test_split(
        ...
        test_targets_tensor = torch.tensor(probe_target_test, dtype=torch.float32).to(
            device
        )
        ...
        print(
            f"  Train R2: {train_r2:.4f}, Test R2: {test_r2:.4f}, Train MSE: {train_loss:.4f}, Test MSE: {test_mse:.4f}"
        )
    ...
    plt.plot(
        layer_numbers, test_r2_scores, "s-", label="Test R²", linewidth=2, markersize=6
    )
    ...
    # Save the graph
    mse_output_filename = (
        "same_dataset_random_split_probe_results_across_layers_mse.png"
    )
    ...
    combined_output_filename = (
        "same_dataset_random_split_probe_results_across_layers_r2_mse.png"
    )
    ...
    print("=" * 50)
    print(
        f"Best Train R2: {max(train_r2_scores):.4f} at layer {layer_numbers[np.argmax(train_r2_scores)]}"
    )
    print(
        f"Best Test R2: {max(test_r2_scores):.4f} at layer {layer_numbers[np.argmax(test_r2_scores)]}"
    )
    ...
    if target_layer not in activations:
        print(
            f"Warning: {target_layer} not found. Available layers: {list(activations.keys())}"
        )
        # Try to find the closest layer
        available_layers = sorted(
            activations.keys(), key=lambda x: int(x.split("_")[1])
        )
        if available_layers:
    ...
    for num_hidden in complexities:
        ...
        train_r2, test_r2, train_mse, test_mse = train_variable_complexity_probe(
            activation_train_tensor,
            activation_test_tensor,
            target_train_tensor,
            target_test_tensor,
        ...
        print(
            f"Train R²: {train_r2:.4f}, Test R²: {test_r2:.4f}, Train MSE: {train_mse:.4f}, Test MSE: {test_mse:.4f}"
        )
    ...
    # Save the graph
    complexity_output_filename = (
        "same_dataset_random_split_probe_results_probe_complexity_layer8.png"
    )
    ...
    # Save the graph
    complexity_mse_output_filename = (
        "same_dataset_random_split_probe_results_probe_complexity_layer8_mse.png"
    )
    ...
    complexity_combined_output_filename = (
        "same_dataset_random_split_probe_results_probe_complexity_layer8_r2_mse.png"
    )
    plt.savefig(complexity_combined_output_filename, dpi=300, bbox_inches="tight")
    print(
        f"Combined complexity R²+MSE graph saved as: {complexity_combined_output_filename}"
    )
    plt.close()
    ...
    print("=" * 50)
    print(
        f"Best Train R2: {max(complexity_train_r2):.4f} with {complexities[np.argmax(complexity_train_r2)]} hidden layer(s)"
    )
    print(
        f"Best Test R2: {max(complexity_test_r2):.4f} with {complexities[np.argmax(complexity_test_r2)]} hidden layer(s)"
    )
