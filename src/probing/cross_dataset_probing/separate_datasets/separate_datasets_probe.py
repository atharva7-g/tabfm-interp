...
def main():
    ...
    for dataset_idx in train_indices:
        ...
        layer_targets = np.full(layer_activations.shape[0], weight_0_value)
    ...
    print(
        f"Target values: {[all_activations[idx]['weights'][0] for idx in train_indices]}"
    )
    ...
    for test_idx in test_indices:
        ...
        # Create target array with same length as activations
        target_values = np.full(layer_activations.shape[0], weight_0_value)
        ...
        print(
            f"  True weight[0]: {weight_0_value}, Predicted: {predictions.mean().item():.4f}"
        )
