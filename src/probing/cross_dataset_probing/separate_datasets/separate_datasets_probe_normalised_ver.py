...
def main():
    ...
    for dataset_idx in train_indices:
        ...
        # Target is the weight[0] value (first weight component)
        weight_0_value = all_activations[dataset_idx]["weights"][0]

        # Compute std devs from the test split used for extracting activations
        X_test_for_norm = all_activations[dataset_idx]["X_test"]
        y_test_for_norm = all_activations[dataset_idx]["y_test"]
        std_x = np.std(X_test_for_norm[:, 0])
        std_y = np.std(y_test_for_norm)
        eps = 1e-8
        normalized_coeff = weight_0_value * (std_x / (std_y + eps))
        layer_targets = np.full(layer_activations.shape[0], normalized_coeff)
    ...
    # Show normalized target values used for probing
    normalized_train_targets = [
        all_activations[idx]["weights"][0]
        * (
            np.std(all_activations[idx]["X_test"][:, 0])
            / (np.std(all_activations[idx]["y_test"]) + 1e-8)
        )
        for idx in train_indices
    ]
    print(f"Target values (normalized coeff): {normalized_train_targets}")
    ...
    for test_idx in test_indices:
        ...
        # Compute normalized target (from the X_test/y_test used when extracting activations)
        weight_0_value = all_activations[test_idx]["weights"][0]
        X_test_for_norm = all_activations[test_idx]["X_test"]
        y_test_for_norm = all_activations[test_idx]["y_test"]
        std_x = np.std(X_test_for_norm[:, 0])
        std_y = np.std(y_test_for_norm)
        print(f"std_x: {std_x}, std_y: {std_y}")
        eps = 1e-8
        normalized_coeff = weight_0_value * (std_x / (std_y + eps))
        # Create target array with same length as activations
        target_values = np.full(layer_activations.shape[0], normalized_coeff)
        ...
        print(
            f"  True weight[0]: {weight_0_value}, Normalized target: {normalized_coeff:.6f}, Predicted: {predictions.mean().item():.6f}"
        )
