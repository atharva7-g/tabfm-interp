...
def log_experiment_params(run: Any, config: Dict[str, Any]) -> None:
	...
	for key, value in config.items():
		if isinstance(value, (list, tuple)):
			...
			# Store nested dicts with dot notation
...
def log_artifact(
		run: Any, artifact_path: Union[str, Path], name: Optional[str] = None
) -> None:
	...
	if suffix in [".png", ".jpg", ".jpeg", ".gif"]:
		...
		run.track(Image(artifact_path), name=name or artifact_path.stem)
	elif suffix == ".json":
		import json
...
class AimExperimentTracker:
	...
	def log_artifacts(
			self, artifact_paths: List[Path], names: Optional[List[str | None]] = None
	):
		if names is None:
