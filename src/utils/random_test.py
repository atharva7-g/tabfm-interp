...
def print_correlations(model):
    ...
    for i in range(m):
        ...
        cosine_similarities.append(
            F.cosine_similarity(
                decoder_layer.weight[i, :], decoder_layer.weight[4000, :], dim=0
            ).item()
        )
    ...
    plt.title("Cosine Similarities between each row and row 4000")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("cosine_similarities.png", dpi=150, bbox_inches="tight")
    plt.close()
    print("Bar graph saved as 'cosine_similarities.png'")
