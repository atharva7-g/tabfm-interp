...
def run_experiment():
    ...
    for i, idx in enumerate(source_indices):
        print(
            f"  Source {i + 1}: a={X_sources[i, 0]:.4f}, b={X_sources[i, 1]:.4f}, True y: {y_sources_true[i]:.4f}"
        )
    ...
    for source_idx, source_num in enumerate(source_indices):
        ...
        for layer_name in layer_names:
            ...
            acts_source_single[layer_name] = all_activations_at_layer[
                :, 3000 + batch_source_idx : 3000 + batch_source_idx + 1, ...
            ]
        ...
        for name in layer_names:
            ...
            y_anchor_patched = apply_activation_patching(
                regressor, model, X_eval, {name: acts_source_single[name]}
            )
            ...
            alignment = (
                pred_change * target_change
            )  # Positive if moving in right direction
            ...
            print(
                f"{name}: base={y_anchor_base:.4f} → patched={y_anchor_patched_val:.4f} "
    ...
    for name in layer_names:
        ...
        print(
            f"{name:>8}  {avg_abs_delta:>10.5f}  {max_abs_delta:>10.5f}  {avg_align:>11.5f}  {pos_align_count:>21}"
        )
    ...
    print("\n" + "-" * 50)
    print(
        f"Most influential layer (highest avg |Δ|): {most_influential[0]}, avg |Δ|={most_influential[1]['avg_abs_delta']:.5f}"
    )
    print(
        f"Best aligned layer (highest avg alignment): {best_aligned[0]}, avg align={best_aligned[1]['avg_alignment']:.5f}"
    )
    ...
    print(
        f"Anchor sample: a={X_anchor[0, 0]:.4f}, b={X_anchor[0, 1]:.4f}, y_true={y_anchor_true:.4f}, y_pred={y_anchor_base:.4f}"
    )
    print("Interpretation guide:")
    print(
        "- Positive alignment: patching source activations moves anchor prediction toward source's true value"
    )
    print(
        "- Negative alignment: patching moves prediction away from source's true value"
    )
    print("- High |Δ|: large change in prediction when patching at this layer")
    print(
        "- Layers with high |Δ| and positive alignment are most informative about feature transfer"
    )
