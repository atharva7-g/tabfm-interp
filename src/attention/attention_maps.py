...
def extract_attention_weights_from_tabpfn(
    model, input_data: np.ndarray, device: str = "cuda"
                    ...
                    if q is not None and k is not None and v is not None:
                        ...
                        if q.dim() == 5:
                            ...
                            q = (
                                q.squeeze(dim=2)
                                if q.shape[2] == 1
                                else q.reshape(q.shape[0], q.shape[1], -1, q.shape[-1])
                            )
                            k = (
                                k.squeeze(dim=2)
                                if k.shape[2] == 1
                                else k.reshape(k.shape[0], k.shape[1], -1, k.shape[-1])
                            )
                            v = (
                                v.squeeze(dim=2)
                                if v.shape[2] == 1
                                else v.reshape(v.shape[0], v.shape[1], -1, v.shape[-1])
                            )
                        elif q.dim() > 4:
                            ...
                            raise ValueError(
                                f"Unexpected q dimension: {q.dim()}, shape: {q.shape}"
                            )

                        # After normalization, q should have shape [batch, seqlen, nhead, d_k]
                        if q.dim() != 4:
                            raise ValueError(
                                f"After processing, q has {q.dim()} dims, expected 4. Shape: {q.shape}"
                            )
                        ...
                        try:
                            ...
                            raise ValueError(
                                f"Cannot unpack q.shape={q.shape} into 4 values: {e}"
                            )
                        ...
                        try:
                            ...
                            raise ValueError(
                                f"Cannot unpack v.shape={v.shape} into 4 values: {e}"
                            )
                        ...
                        dropout_p = (
                            attn_module.dropout_p
                            if attn_module.dropout_p is not None
                            else 0.0
                        )
                        if dropout_p > 0.0:
                            attention_probs = torch.dropout(
                                attention_probs, dropout_p, train=False
                            )

                        # Store attention weights: shape [batch, seq_q, seq_k, nhead]
                        # Reshape to [batch, nhead, seq_q, seq_k] for easier visualization
                        attention_probs = attention_probs.permute(
                            0, 3, 1, 2
                        )  # [b, q, k, h] -> [b, h, q, k]
    ...
    for i, layer in enumerate(model.model_.transformer_encoder.layers):
        ...
        if hasattr(layer, "self_attn_between_features") and hasattr(
            layer, "self_attn_between_items"
        ):
            handle1 = layer.self_attn_between_features.register_forward_hook(
                attention_hook(f"layer_{i}_features")
            )
            handle2 = layer.self_attn_between_items.register_forward_hook(
                attention_hook(f"layer_{i}_items")
            )
...
def visualize_attention_heads(
    ...
    if len(input_data.shape) == 2 and input_data.shape[0] > sample_idx:
        ...
        ]  # Select single sample: [1, num_features]
        print(
            f"Visualizing attention for sample {sample_idx} (selecting from {input_data.shape[0]} samples)"
        )
    else:
        input_data_for_extraction = input_data
        if len(input_data.shape) == 2:
            print(
                f"Visualizing attention for all {input_data.shape[0]} samples (sample_idx {sample_idx} >= num_samples)"
            )
        else:
    ...
    attention_weights = extract_attention_weights_from_tabpfn(
        model, input_data_for_extraction, device
    )

    if not attention_weights:
        print(
            "Warning: No attention weights extracted. The model might not have accessible attention mechanisms."
        )
        return
    ...
    features_attention = {
        k: v for k, v in attention_weights.items() if "_features" in k
    }
    ...
    items_layer_names = sorted(
        [k for k in items_attention.keys() if is_layer_1_or_2(k)]
    )
    features_layer_names = sorted(
        [k for k in features_attention.keys() if is_layer_1_or_2(k)]
    )
...
def visualize_items_attention(
    ...
    """
    print("Visualizing items attention for each head separately...")
    ...
    print(
        f"Showing all {batch_size} batch samples and {num_heads} heads for items attention"
    )
    ...
    for head_idx in range(num_heads):
        ...
        fig, axes = plt.subplots(
            batch_size, num_layers, figsize=(num_layers * 4, batch_size * 3)
        )
        ...
        for layer_idx in range(num_layers):
            ...
            for batch_idx in range(batch_size):
                ...
                if len(attn_map.shape) > 2:
                    print(
                        f"Warning: Unexpected attention map shape {attn_map.shape} for layer {layer_name}, batch {batch_idx}, head {head_idx}, flattening extra dimensions"
                    )
                    while len(attn_map.shape) > 2:
                        attn_map = attn_map.squeeze()
                elif len(attn_map.shape) < 2:
                    print(
                        f"Warning: Unexpected attention map shape {attn_map.shape} for layer {layer_name}, batch {batch_idx}, head {head_idx}"
                    )
                ...
                im = ax.imshow(
                    ...
                    vmin=0,
                    vmax=1,
                    extent=[-0.5, seq_k - 0.5, seq_q - 0.5, -0.5],
                    origin="upper",
                )
                ...
                if batch_idx == 0:
                    ax.set_title(
                        f"Layer {layer_idx + 1}, Head {head_idx + 1}", fontsize=12
                    )
                if layer_idx == 0:
...
def visualize_features_attention(
    attention_weights: Dict[str, torch.Tensor],
    layer_names: List[str],
    num_layers: int,
    sample_idx: int,
    ...
    Args:
            ...
            filename: Name pattern for the output files (will be modified with head number)
    """
    print("Visualizing features attention for each head separately...")
    ...
    for head_idx in range(num_heads):
        ...
        for layer_idx in range(num_layers):
            ...
            if len(attn_map.shape) > 2:
                print(
                    f"Warning: Unexpected attention map shape {attn_map.shape} for layer {layer_name}, head {head_idx}, flattening extra dimensions"
                )
                while len(attn_map.shape) > 2:
                    attn_map = attn_map.squeeze()
            elif len(attn_map.shape) < 2:
                print(
                    f"Warning: Unexpected attention map shape {attn_map.shape} for layer {layer_name}, head {head_idx}"
                )
            ...
            # Set title and labels
            ax.set_title(
                f"Layer {layer_idx + 1} Features, Head {head_idx + 1} (Avg over Items)",
                fontsize=12,
            )
...
def create_sample_data_for_visualization(
    num_samples: int = 10, num_features: int = 4
) -> np.ndarray:
    """
...
def create_mock_attention_visualization(
    input_data: np.ndarray,
    output_dir: str = ".",
    filename: str = "attentions.png",
    num_layers: int = 6,
    ...
    fig, axes = plt.subplots(
        num_layers, num_heads, figsize=(num_heads * 2.5, num_layers * 2.5)
    )
    ...
    for layer_idx in range(num_layers):
        for head_idx in range(num_heads):
            ...
            if layer_idx == 0:
                ...
                attn_map = np.eye(seq_len) * 0.5 + np.random.uniform(
                    0.05, 0.15, (seq_len, seq_len)
                )
            else:
