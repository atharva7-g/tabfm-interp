...
def extract_attention_weights_from_tabpfn(model, input_data: np.ndarray, device: str = 'cuda') -> Dict[
	str, torch.Tensor]:
	"""
				...
				try:
					...
					if q is not None and k is not None and v is not None:
						...
						if q.dim() == 5:
							...
							q = q.squeeze(dim=2) if q.shape[2] == 1 else q.reshape(q.shape[0], q.shape[1], -1,
							                                                       q.shape[-1])
							k = k.squeeze(dim=2) if k.shape[2] == 1 else k.reshape(k.shape[0], k.shape[1], -1,
							                                                       k.shape[-1])
							v = v.squeeze(dim=2) if v.shape[2] == 1 else v.reshape(v.shape[0], v.shape[1], -1,
							                                                       v.shape[-1])
						elif q.dim() > 4:
