#!/usr/bin/env python3
"""Option E: steer full 192-d items MHA output at Layer 0."""
...
METHOD_NAME = "items_attention"
LAYER_IDX = 0


def _get_mha(model: TabPFNRegressor) -> MultiHeadAttention:
    return model.model_.transformer_encoder.layers[LAYER_IDX].self_attn_between_items
...
def _run_with_steering(
    ...
    def _transform(output: torch.Tensor, _x: torch.Tensor, _kwargs: dict) -> torch.Tensor:
        ...
        if output.shape[1] == delta_local.shape[0]:
...
def main() -> None:
    ...
    fig_path = Path("docs/attention_head_patching/steering_items_attention.png")
    plot_results(
        results,
        "Steering E: Full 192-d Items MHA Output at Layer 0",
        fig_path,
        baseline_mse,
    )
    ...
    config = {
        ...
        "hook": "layer_0.self_attn_between_items._compute",
        "n_eval": int(N_EVAL),
        "layer_idx": LAYER_IDX,
    ...
    print_results_summary("E", results)
