...
from src.attention.attention_maps import (
    extract_attention_weights_from_tabpfn,
...
def test_attention_extraction():
    ...
    try:
        attention_weights = extract_attention_weights_from_tabpfn(
            regressor, X_test, device
        )

        if attention_weights:
            print(
                f"Successfully extracted attention weights from {len(attention_weights)} layers"
            )
