...
def logit_lens(X_test: np.ndarray, y_test: np.ndarray, regressor: TabPFNRegressor, borders: List[np.ndarray],
               predictions: Dict[str, np.ndarray], dataset_index: int):
	"""Test the accuracy of the model on the datasets"""
	predictions = {}
	...
	make_graph(logit_lens_list, y_test_list, dataset_index=dataset_index,
	           title=f"Dataset {dataset_index + 1} predictions vs target")
...
def main():
	...
	weights_list = [[1.0, 1.0], [2.0, 2.0], [3.0, 3.0], [4.0, 4.0], [5.0, 5.0], [6.0, 6.0], [7.0, 7.0], [8.0, 8.0],
	                [9.0, 9.0], [10.0, 10.0]]
