...
def train_linear_probe(activations: torch.Tensor, targets: torch.Tensor, 
    ...
    # Initialize probe
    probe = LinearProbe(input_dim=activation_size, output_dim=2, hidden_dim=256).to(device)
    ...
    with torch.no_grad():
        ...
        r2_alpha = r2_score(target_test[:, 0].cpu().numpy(), test_outputs[:, 0].cpu().numpy())
        r2_beta = r2_score(target_test[:, 1].cpu().numpy(), test_outputs[:, 1].cpu().numpy())
        r2 = (r2_alpha + r2_beta) / 2
    
    return probe, test_loss.item(), r2

def test_single_layer(activations_data, train_indices, test_indices, layer_name, device='cuda'):
    ...
    for dataset_idx in train_indices:
        ...
        layer_targets = np.full((layer_activations.shape[0], 2), [target_alpha, target_beta])
    ...
    for test_idx in test_indices:
        ...
        # Create target array with same length as activations, with 2D targets
        target_values = np.full((layer_activations.shape[0], 2), [target_alpha, target_beta])
    ...
    print(f"  Average Test Loss: {avg_test_loss:.4f}, Average Test R2: {avg_test_r2:.4f}")
...
def main():
    ...
    for layer_name in available_layers:
        try:
            result = test_single_layer(all_activations, train_indices, test_indices, layer_name, device)
    ...
    if layer_results:
        ...
        # Sort by R2 score
        layer_results_sorted = sorted(layer_results, key=lambda x: x['avg_test_r2'], reverse=True)
        ...
        for result in layer_results_sorted:
            print(f"{result['layer']:<12} {result['train_r2']:<10.4f} {result['avg_test_r2']:<10.4f} {result['avg_test_loss']:<12.4f}")
        ...
        print(f"  Individual Test R² scores: {[f'{r:.4f}' for r in best_layer_by_r2['test_r2s']]}")
